...
class DTLSHandshake:
    ...
    def create_client_certificate_with_blockhash(self):
        """Create client certificate message with block hash"""
        try:
            cert_body = json.dumps(self.pub_key).encode()
            block_hash = self.blockchain.chain[-1].hashid  # Assuming the last block is the one with the certificate
            ...
            blockhash_msg = self.handshake.create_handshake_header(
                msg_type=HandshakeType.MTACK,  # Custom MTACK message type
                length=len(block_hash),
                seq=2
            ) + block_hash

            record = self.record_layer.create_record_header(
                content_type=22,  # Handshake
                length=len(cert_msg) + len(blockhash_msg)
            ) + cert_msg + blockhash_msg

            print(f"Created Certificate and BlockHash ({len(record)} bytes)")
    ...
    def send_client_certificate_with_blockhash(self):
        """Send client certificate and block hash to server"""
        try:
            cert_record = self.create_client_certificate_with_blockhash()
            response = self.send_packet(cert_record)
            if not response:
                print("No response to Certificate and BlockHash")
                return False
            return True

        except Exception as e:
            print(f"Error sending client certificate and block hash: {e}")
            return False
    ...
    def run(self):
        ...
        try:
            if self.perform_handshake():
                ...
                request_handler.get_request_udp(server_ip="192.168.0.197", port=5000, server_pub=self.server_certificate)

...
qrng = b'\xb8\x08\x9c\xd2\xea\xbe\xf7\x1e\xac\xf1\xd6$'
...
ack = IP(dst=dst_ip) / TCP(sport=src_port, reserved=5, dport=dst_port, flags="A", ack=syn_ack.seq + 1, seq=1)/Raw(load=qrng)
...
pub = base64.b64decode(data_frags)
ct, shared = frodo.kem_encaps(pub)
encoded_ct = base64.b64encode(ct)

# create a TCP data packet with the message to send to the server
server_fragments = fragment_message(encoded_ct)
...
cipher = cip.encrypt(qrng, b"test YA BROO")
print("cipher is ",cipher)
pkt = IP(dst=dst_ip, ) / TCP(sport=src_port, dport=dst_port, flags="P", ack=last_ack[TCP].seq + 1, seq = last_ack[TCP].ack, ) / Raw(load=cipher)
pkt = s.sr1(pkt)
print("receiving response")
print(pkt[Raw].load)

# Get the IP option value
ip_opt = pkt[IP].options[0].value
print("ip o : ",ip_opt)
# Get the TCP option value
tcp_opt = pkt[TCP].options[0][1]
print("tcp o: ",tcp_opt)
# Concatenate the two halves
server_blockchain = tcp_opt + ip_opt

# Print blockchain
print("blockchain enc: ", server_blockchain)
nonce = qrng
plaintext_blk = cip.decrypt(nonce, bytearray(server_blockchain))
print("plaintext blockchain ", plaintext_blk)
# Create the FIN packet
fin = IP(dst=dst_ip) / TCP(sport=src_port, dport=dst_port, flags="F", ack=pkt[TCP].seq + 1, seq = pkt[TCP].ack)
print("sending FA")
